...
//                .collect(Collectors.toList());
        ...
        List<ProjectRepository.ProjectWithRole> projectWithRole = projectRepository.findAllProjectAccessibleByUser(userId);
        return projectWithRole.stream()
                .map(projectWithRole1 ->
                    projectMapper.toProjectSummaryResponse(projectWithRole1.getProject(),projectWithRole1.getRole())
                )
                .toList();
    }
    ...
    public ProjectSummaryResponse getUserProjectById( Long projectId) {
        Long userId = authUtil.getCurrentUserId();
        ProjectRepository.ProjectWithRole projectWithRole = projectRepository.findAccessibleProjectByIdWithRole(projectId,userId).orElseThrow(()-> new BadRequestException("Project Not Found"));
